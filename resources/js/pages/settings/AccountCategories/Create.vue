...
const submit = async () => {
  form.post('/settings/account-categories', {
    onStart: () => {
      // info('Menyimpan kategori...', {
      //   message: 'Sedang memproses data kategori baru.',
      //   duration: 2000
      // });
    },
    onSuccess: () => {
      success('Kategori berhasil dibuat!', {
        message: 'Kategori rekening baru telah ditambahkan ke daftar kategori Anda.',
        duration: 5000
      });
    },
    onError: (errors) => {
      ...
      if (errors.name) {
        error('Nama kategori diperlukan', {
          message: 'Silakan masukkan nama untuk kategori baru.',
          duration: 4000
        });
      } else {
        error('Gagal membuat kategori', {
          message: 'Terjadi kesalahan saat menyimpan data kategori. Silakan coba lagi.',
          persistent: true
        });
      }
    }
...
<template>
  <FinancialAppLayout :showHeader="false" :showFab="false">
    <Head title="Tambah Kategori Rekening" />
    ...
    <div class="mb-8">
      ...
      <div class="flex items-center gap-3 mb-2">
        ...
        <h1 class="text-3xl font-bold bg-gradient-to-r from-teal-600 to-coral-600 bg-clip-text text-transparent">
          Tambah Kategori Rekening
        </h1>
      </div>
      <p class="text-gray-600 dark:text-gray-300">
        Buat kategori baru untuk mengorganisir rekening Anda. Kategori membantu Anda mengelompokkan rekening berdasarkan jenisnya.
      </p>
    </div>
