...
const search = () => {
  router.get('/settings/transaction-categories', { 
...
const deleteCategory = async (category: TransactionCategory) => {
  ...
  if (confirmed) {
    router.delete(`/settings/transaction-categories/${category.id}`, {
      onStart: () => {
...
<template>
  <FinancialAppLayout :showHeader="false" :showFab="false">
      ...
      <div class="flex flex-col sm:flex-row gap-4">
        ...
        <div class="flex items-center gap-2">
          <select
            v-model="typeFilter"
            @change="search"
            class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-transparent"
          >
    ...
    <div v-if="categories.data.length === 0" class="text-center py-16">
      ...
      <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">Belum ada kategori transaksi</h3>
      <p class="text-gray-600 dark:text-gray-400 mb-6">Mulai dengan menambahkan kategori transaksi pertama Anda</p>
      <Link
        href="/settings/transaction-categories/create"
    ...
    <div v-else-if="filteredAndSortedCategories.length === 0" class="text-center py-12">
      ...
      <p class="text-gray-600 dark:text-gray-400">Coba ubah kata kunci pencarian atau filter Anda</p>
    </div>
        ...
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-3" @click="handleSelectCategory(category.id)">
            <div class="w-8 h-8 bg-gradient-to-r from-teal-100 to-coral-100 dark:from-teal-900 dark:to-coral-900 rounded-md flex items-center justify-center">
              <component 
                :is="idHasBeenSelected(category.id) ? Check : getTypeIcon(category.type)" 
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mb-1">
                ...
                <span v-if="category.is_hide" class="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200">
                  Tersembunyi
                </span>
              </div>
            </div>
