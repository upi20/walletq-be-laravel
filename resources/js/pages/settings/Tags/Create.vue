...
const submit = async () => {
  form.post('/settings/tags', {
    onStart: () => {
      // info('Menyimpan tag...', {
      //   message: 'Sedang memproses data tag baru.',
      //   duration: 2000
      // });
    },
    onSuccess: () => {
      success('Tag berhasil dibuat!', {
        message: 'Tag baru telah ditambahkan ke daftar tag Anda.',
        duration: 5000
      });
    },
    onError: (errors) => {
      ...
      if (errors.name) {
        error('Nama tag diperlukan', {
          message: 'Silakan masukkan nama untuk tag baru.',
          duration: 4000
        });
      } else {
        error('Gagal membuat tag', {
          message: 'Terjadi kesalahan saat menyimpan data tag. Silakan coba lagi.',
          persistent: true
        });
      }
    }
  ...
  <FinancialAppLayout :showHeader="false" :showFab="false">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-600 dark:text-gray-300">
        Buat tag baru untuk menandai dan mengkategorikan transaksi Anda. Tag membantu Anda menemukan transaksi dengan mudah.
      </p>
    </div>
