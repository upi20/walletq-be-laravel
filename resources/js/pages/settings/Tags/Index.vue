...
const toggleMenu = (tagId: number) => {
  activeMenu.value = activeMenu.value === tagId ? null : tagId;
};
...
const filteredAndSortedTags = computed(() => {
  ...
  return Array.isArray(props.tags.data) ? props.tags.data.slice() : [];
});
    ...
    router.delete(`/settings/tags/${tag.id}`, {
      ...
      onSuccess: () => {
        success('Tag berhasil dihapus!', {
          message: `Tag "${tag.name}" telah dihapus dari sistem.`,
          duration: 5000
        });
      },
      onError: () => {
        error('Gagal menghapus tag', {
          message: 'Terjadi kesalahan saat menghapus tag. Silakan coba lagi.',
          persistent: true
        });
      }
    });
...
const idHasBeenSelected = (tagId: number) => {
  return selectedTagsIds.value.includes(tagId);
};
...
<template>
  <FinancialAppLayout :showHeader="false" :showFab="false" containerClass="px-3 pt-6 pb-32">
        ...
        <div>
          <div class="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <h3 class="text-xs">
                Kelola tag untuk menandai transaksi Anda
              </h3>
            </div>
          </div>
        </div>
    ...
    <!-- Tags List -->
    <div v-if="tags.data.length === 0" class="text-center py-16">
      <div class="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-6">
        <Hash class="w-12 h-12 text-gray-400" />
      </div>
      <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">Belum ada tag</h3>
    ...
    <div v-else class="grid grid-cols-1 gap-3 mb-20">
      <div
        v-for="(tag, index) in filteredAndSortedTags"
        :key="tag.id"
        :class="[
